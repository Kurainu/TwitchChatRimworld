...
internal static class ChatNamer
{
	...
	// re-pick (the tab button): link a random recent chatter that's free under the reuse rule.
	// returns false when nothing is free so the tab can show why in place.
	public static bool ForceRename(Pawn pawn)
	{
		if (pawn == null) return false;
		reuseNames = LoadedModManager.GetMod<TwitchChatSettingsMod>().GetSettings<Settings.Settings>().reuseNames;
		string login = FreeLogin(pawn, ChatterPool.Snapshot().InRandomOrder());
	...
	public static void Tick()
	{
		...
		// pull settings first so GiveNames below acts on the current toggles, not last tick's
	...
	private static bool Eligible(Pawn pawn)
	{
		...
		if (pawn.AnimalOrWildMan()) return player ? nameTameAnimals : (pawn.Faction == null ? nameWildAnimals : nameOther);
	...
	{
		string login = FreeLogin(pawn, ChatterPool.ConsumedSnapshot().InRandomOrder());
		if (login == null) return false;
	...
	// first candidate free for this pawn, or null if none
	private static string FreeLogin(Pawn pawn, IEnumerable<string> candidates)
	{
		foreach (string login in candidates)
			if (CanAssign(pawn, login, reuseNames)) return login;
		return null;
	}

	// can this login go on the pawn: never duplicated within its own kind (animal vs not), and not across kinds unless reuse is on
	private static bool CanAssign(Pawn pawn, string login, bool reuse)
	{
		bool animal = pawn.AnimalOrWildMan();
		bool usedBySameKind = LinkedMatches(login, animal);
		bool usedByOtherKind = LinkedMatches(login, !animal);
		return !usedBySameKind && (reuse || !usedByOtherKind);
	}

	// what the picker offers: the same rule as the random button, so it can't create a name the setting forbids
	public static IEnumerable<string> Pickable(Pawn pawn, IEnumerable<string> logins)
	{
		bool reuse = LoadedModManager.GetMod<TwitchChatSettingsMod>().GetSettings<Settings.Settings>().reuseNames;
		return logins.Where(login => CanAssign(pawn, login, reuse));
	}
